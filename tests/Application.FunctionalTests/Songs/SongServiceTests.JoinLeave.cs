...
public partial class SongServiceTests
{
    ...
    public async Task Join_AddsAssignment()
    {
        ...
        result.Roles.Count(r => r.Assignment != null).ShouldBe(1);
        result.Roles.Single(r => r.Title == "Вокал").Assignment.ShouldNotBeNull();
        result.Roles.Single(r => r.Title == "Вокал").Assignment!.User.Id.ShouldBe(appUser.Id);

        using var db = Db();
        (await db.SongRoleAssignments.CountAsync(
            a => a.SongId == songId && a.RoleId == roleId && a.UserId == appUser.Id)).ShouldBe(1);
    }

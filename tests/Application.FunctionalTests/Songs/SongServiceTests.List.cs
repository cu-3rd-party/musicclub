...
public partial class SongServiceTests
{
    ...
    public async Task List_Empty_ReturnsEmptyResult()
    {
        ...
        var result = await scope.Songs.ListAsync(null, 20, null, user, CancellationToken.None);
    ...
    public async Task List_Query_MatchesTitleOrArtist_CaseInsensitive()
    {
        ...
        using var scope = new SongScope();
        var result = await scope.Songs.ListAsync("avenged", 20, null, user, CancellationToken.None);
    ...
    public async Task List_Query_NoMatches_ReturnsEmpty()
    {
        ...
        using var scope = new SongScope();
        var result = await scope.Songs.ListAsync("zzz-nothing", 20, null, user, CancellationToken.None);
    ...
    public async Task List_Pagination_ReturnsNextPageTokenAndSecondPage()
    {
        ...
        using var scope = new SongScope();
        var page1 = await scope.Songs.ListAsync(null, 2, null, user, CancellationToken.None);
        page1.Songs.Select(s => s.Id).ShouldBe(new[] { newestId, middleId });
        page1.NextPageToken.ShouldBe("2");

        var page2 = await scope.Songs.ListAsync(null, 2, page1.NextPageToken, user, CancellationToken.None);
    ...
    public async Task List_InvalidPageSize_FallsBackToDefault()
    {
        ...
        using var scope = new SongScope();
        var result = await scope.Songs.ListAsync(null, 0, null, user, CancellationToken.None);
        result.Songs.Count.ShouldBe(5);
        result.NextPageToken.ShouldBeNull();

        result = await scope.Songs.ListAsync(null, 500, null, user, CancellationToken.None);
        result.Songs.Count.ShouldBe(5);
    }
    ...
    public async Task List_InvalidPageToken_FallsBackToZero()
    {
        ...
        using var scope = new SongScope();
        var result = await scope.Songs.ListAsync(null, 1, "not-a-number", user, CancellationToken.None);
    ...
    public async Task List_AssignmentCount_CountsDistinctRoles()
    {
        ...
        using (var scope = new SongScope())
        {
            await scope.Songs.JoinRoleAsync(songId, "Вокал", owner, CancellationToken.None);
            await scope.Songs.JoinRoleAsync(songId, "Вокал", member, CancellationToken.None);
        }

        using (var scope = new SongScope())
        {
            var result = await scope.Songs.ListAsync(null, 20, null, owner, CancellationToken.None);
            result.Songs.Single().AssignmentCount.ShouldBe(1);
        }
    }
}
